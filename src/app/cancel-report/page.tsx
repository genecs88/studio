...
export default function CancelReportPage() {
  ...
  const handleCreateJson = () => {
    ...
    if (env && cancelAction) {
        setConstructedPostUrl(`${env.url}${cancelAction.value}`);
    } else {
        setConstructedPostUrl("");
    }
  };
  ...
  return (
    <div className="flex flex-col gap-6">
                ...
                >
                    {environments.map((env) => (
                        <div key={env.id} className="flex items-center space-x-2">
                            <RadioGroupItem value={env.id} id={`cancel-env-${env.id}`} />
                            <Label htmlFor={`cancel-env-${env.id}`} className="font-normal cursor-pointer">
                                {env.name}
                            </Label>
                        </div>
                    ))}
      ...
      <div className="mt-2">
        <Button onClick={handleCancelReport} disabled={isLoading || !jsonPayload} size="lg">
            {isLoading ? 'Canceling...' : 'Cancel Report'}
        </Button>
      </div>
