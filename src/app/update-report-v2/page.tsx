...
export default function UpdateReportV2Page() {
    ...
    const [updateResponse, setUpdateResponse] = useState<any | null>(null);
    const [isUpdateLoading, setIsUpdateLoading] = useState(false);
    const [constructedPostUrl, setConstructedPostUrl] = useState("");
    
    const { toast } = useToast();

    // Automatically update the JSON payload when identifiers or status change
    useEffect(() => {
    ...
    const handleUpdate = async () => {
        ...
        if (!environment) {
            setUpdateResponse(`Error: Environment configuration for '${env}' not found.`);
            setIsUpdateLoading(false);
            return;
        }
        ...
        if (!updateAction) {
            setUpdateResponse("Error: 'UPDATE' API action not configured.");
            setIsUpdateLoading(false);
            return;
        }
        ...
        try {
            ...
            setUpdateResponse("Error: JSON Payload is not valid JSON.");
            setIsUpdateLoading(false);
            return;
        }

        if (!parsedPayload.status) {
            setUpdateResponse("Error: Status is required in the JSON payload. Please select a status.");
            setIsUpdateLoading(false);
            return;
        }
        ...
        if (!organization) {
            setUpdateResponse(`Error: Organization '${extractedParentOrg}' not found in environment '${environment.name}'.`);
            setIsUpdateLoading(false);
            return;
        }
        ...
        if (!apiKeyData) {
            setUpdateResponse(`Error: API Key for organization '${organization.name}' in environment '${environment.name}' not found.`);
            setIsUpdateLoading(false);
            return;
        }

        const urlToFetch = `${environment.url}${updateAction.value}`;
        ...
        try {
            ...
            try {
                setUpdateResponse(JSON.parse(responseText));
            } catch (e) {
    ...
    return (
        <div className="flex flex-col gap-6">
            ...
            <Card>
                <CardHeader>
                    <CardTitle>Step 2: Update Details</CardTitle>
                    <CardDescription>Select a new status and confirm the payload.</CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                    <div className="pt-2">
                        <Label className="font-semibold">New Status</Label>
                        <RadioGroup
                            value={status}
            ...
            <div className="grid md:grid-cols-4 gap-6">
                <Card className="md:col-span-1">
                    <CardHeader>
                        ...
                        <CardDescription>This payload will be sent to the update endpoint.</CardDescription>
                    </CardHeader>
                    <CardContent>
                        <Textarea
                            value={jsonPayload}
                ...
                <Card className="md:col-span-3">
                    <CardHeader>
                        <CardTitle>API Response (from Update)</CardTitle>
                        <CardDescription>The response from the Update POST request will appear here.</CardDescription>
                    </CardHeader>
                    <CardContent>
                        {isUpdateLoading ? (
                            ...
                            <Textarea
                                ...
                                readOnly
                                rows={15}
                                placeholder="Update API response will be shown here."
                                className="font-mono text-sm"
                            />
                        )}
                    </CardContent>
            ...
            <div className="mt-2">
                <Button onClick={handleUpdate} disabled={isUpdateLoading || !jsonPayload || !status} size="lg">
                    {isUpdateLoading ? 'Updating...' : 'Update Report Status'}
                </Button>
            </div>
