...
export default function CancelReportv2Page() {
    ...
    const handleCancelReport = async () => {
        ...
        if (!environment) {
            setCancelResponse(`Error: Environment configuration for '${env}' not found in Admin Management.`);
            setIsCancelLoading(false);
            return;
        }

        const cancelAction = apiActions.find(a => a.key === 'CANCEL');
        if (!cancelAction) {
            setCancelResponse("Error: 'CANCEL' API action not configured.");
            setIsCancelLoading(false);
            return;
        }
        ...
        try {
            ...
            setCancelResponse("Error: JSON Payload is not valid JSON.");
            setIsCancelLoading(false);
            return;
        }
        ...
        if (!organization) {
            setCancelResponse(`Error: Organization '${extractedParentOrg}' not found in environment '${environment.name}'.`);
            setIsCancelLoading(false);
            return;
        }
        ...
        if (!apiKeyData) {
            setCancelResponse(`Error: API Key for organization '${organization.name}' in environment '${environment.name}' not found.`);
            setIsCancelLoading(false);
            return;
        }

        const urlToFetch = `${environment.url}${cancelAction.value}`;
        ...
        try {
            ...
            try {
                setCancelResponse(JSON.parse(responseText));
            } catch (e) {
                setCancelResponse(responseText);
            }
        } catch (error: any) {
            setCancelResponse(`Error: ${error.message}`);
        } finally {
            setIsCancelLoading(false);
        }
    };
    ...
    return (
        <div className="flex flex-col gap-6">
            <div>
                ...
                <p className="text-muted-foreground">Search Datadog to get a report payload, then use it to cancel the report.</p>
            </div>
            ...
            {/* Step 2: Cancel Report */}
            <div className="grid md:grid-cols-4 gap-6">
                <Card className="md:col-span-1">
                    <CardHeader>
                        <CardTitle>Step 2: JSON Payload</CardTitle>
                        <CardDescription>This payload will be sent to the CANCEL endpoint.</CardDescription>
                    </CardHeader>
                    <CardContent>
                        <Textarea
                            value={jsonPayload}
                ...
                <Card className="md:col-span-3">
                    <CardHeader>
                        <CardTitle>API Response (from CANCEL)</CardTitle>
                        <CardDescription>The response from the CANCEL POST request will appear here.</CardDescription>
                    </CardHeader>
                    <CardContent>
                        {isCancelLoading ? (
                            ...
                            <Textarea
                                ...
                                readOnly
                                rows={12}
                                placeholder="CANCEL API response will be shown here."
                                className="font-mono text-sm"
                            />
                        )}
                    </CardContent>
            ...
            <div className="mt-2">
                <Button onClick={handleCancelReport} disabled={isCancelLoading || !jsonPayload} size="lg">
                    {isCancelLoading ? 'Canceling...' : 'Cancel Report'}
                </Button>
            </div>
