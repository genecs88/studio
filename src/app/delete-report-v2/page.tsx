...
export default function DeleteReportv2Page() {
    ...
    const handleDeleteReport = async () => {
        ...
        if (!environment) {
            setDeleteResponse(`Error: Environment configuration for '${env}' not found in Admin Management.`);
            setIsDeleteLoading(false);
            return;
        }

        const deleteAction = apiActions.find(a => a.key === 'DELETE');
        if (!deleteAction) {
            setDeleteResponse("Error: 'DELETE' API action not configured.");
            setIsDeleteLoading(false);
            return;
        }
        ...
        try {
            ...
            setDeleteResponse("Error: JSON Payload is not valid JSON.");
            setIsDeleteLoading(false);
            return;
        }
        ...
        if (!organization) {
            setDeleteResponse(`Error: Organization '${extractedParentOrg}' not found in environment '${environment.name}'.`);
            setIsDeleteLoading(false);
            return;
        }
        ...
        if (!apiKeyData) {
            setDeleteResponse(`Error: API Key for organization '${organization.name}' in environment '${environment.name}' not found.`);
            setIsDeleteLoading(false);
            return;
        }

        const urlToFetch = `${environment.url}${deleteAction.value}`;
        ...
        try {
            ...
            try {
                setDeleteResponse(JSON.parse(responseText));
            } catch (e) {
    ...
    return (
        <div className="flex flex-col gap-6">
            ...
            <div className="grid md:grid-cols-4 gap-6">
                <Card className="md:col-span-1">
                    <CardHeader>
                        ...
                        <CardDescription>This payload will be sent to the DELETE endpoint.</CardDescription>
                    </CardHeader>
                    <CardContent>
                        <Textarea
                            value={jsonPayload}
                ...
                <Card className="md:col-span-3">
                    <CardHeader>
                        <CardTitle>API Response (from DELETE)</CardTitle>
                        <CardDescription>The response from the DELETE POST request will appear here.</CardDescription>
                    </CardHeader>
                    <CardContent>
                        {isDeleteLoading ? (
                            ...
                            <Textarea
                                ...
                                readOnly
                                rows={12}
                                placeholder="DELETE API response will be shown here."
                                className="font-mono text-sm"
                            />
                        )}
                    </CardContent>
            ...
            <div className="mt-2">
                <Button onClick={handleDeleteReport} disabled={isDeleteLoading || !jsonPayload} size="lg" variant="destructive">
